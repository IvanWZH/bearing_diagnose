...
#加载训练好的模型
clf = joblib.load(r'D:\py-program\phm_zhoucheng\model\final\(90.84)LGB_ne400_md10_nl1023_ff0.8(IF0.0005-0.0001).model')

#获取原始测试集路径下各个文件的文件名并排序
dirs_test = os.listdir(r'第二批测试数据所在文件夹（cwru文件夹）的路径')
dirs_test.sort(key=lambda x: int(x.split('T')[2][:-4]))
#获得各个原始测试文件的路径列表，此处放原始数据所在文件夹的路径
inputfile_path = [os.path.join(r'第二批测试数据所在文件夹的路径（cwru文件夹）', name) for name in dirs_test]
#数据清洗后的文件保存路径列表，此处放cleandata文件夹的路径
cleandata_path = [os.path.join(r'cleandata文件夹的路径',name) for name in dirs_test]
#提取特征后的文件保存路径列表，此处放feature文件夹的路径
feature_path  = [os.path.join(r'feature文件夹的路径',name) for name in dirs_test]

label = ['label']
filename = ['filename']

for i in range(0,142):
    datacleaning_if(inputfile_path[i], cleandata_path[i])#数据清洗
    feature_get_new(cleandata_path[i],feature_path[i],400)#特征提取

    feature = []#特征列表
    csv_file = csv.reader(open(feature_path[i]))
    cnt = 0
    ...
    label_pred.sort()
    label_pred[:] = collections.Counter(label_pred).most_common(1)[0][0]#将出现频率最高的label作为预测结果
    print(label_pred)
...
#此处设置输出结果文件的路径
result_df.to_csv(r'预测结果csv文件的路径',index=None,header=None)
